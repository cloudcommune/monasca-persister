...
if [ $requirements_installed -eq 0 ]; then
    ...
    $ZUUL_CLONER --cache-dir \
        /opt/git \
        --branch $BRANCH_NAME \
        ssh://jenkins@gerrit.yovole.tech:29418 \
    ...
    if [ -z "$REQUIREMENTS_PIP_LOCATION" ]; then
        REQUIREMENTS_PIP_LOCATION="git+ssh://jenkins@gerrit.yovole.tech:29418/openstack/requirements@$BRANCH_NAME#egg=requirements"
    fi
